...
#load data
raw <- read.table("GSE120575_Sade_Feldman_melanoma_single_cells_TPM_GEO.txt.gz",sep = "\t",header = F,skip = 2)
rownames(raw) <- raw[,1]
raw <- raw[,-1]
raw <- raw[,-nrow(raw)]
raw[1:10,1:10]
cellname <- read.table("GSE120575_Sade_Feldman_melanoma_single_cells_TPM_GEO.txt.gz",sep = "\t",header = F,nrows = 2)
cellname <- cellname[,-1]
colnames(raw) <- cellname[1,]
cellname <- cellname[,which(str_sub(cellname[2,],1,3) == "Pre")]

cellname <- as.data.frame(t(cellname))
table(cellname$V2)

raw <- raw[,cellname$V1]
raw <- as.data.frame(t(raw))
plot(raw$CD8A,raw$CD4)
raw <- raw[which(raw$CD8A > 0 & raw$CD8B > 0 & raw$CD4 == 0),geneused]
...
write.csv(proliferation,file = "ProliferationMm_gene.csv")
write.csv(acquisitiongene,file = "Chol_Acquisition_gene.csv")
write.csv(plotdata,file = "Mm_CD8+_plotdata_fin_20220519.csv")
