...
proliferation <- read.table("ProliferationNSCLC_gene.csv",header = T,sep = ",")
proliferation <- proliferation[,2]

#read data and basic treatment
raw <- read.table("GSE99254_NSCLC.TCell.S12346.TPM.txt.gz",sep = "\t",header = T)
raw[1:5,1:5]
raw <- raw[which(!duplicated(raw$symbol)),]
raw <- na.omit(raw)
rownames(raw) <- raw$symbol
raw <- raw[,-(1:2)]
raw <- as.data.frame(t(raw))
save(raw,file = "ZZM_NSCLC_TPMdata.RData")

#load data and set filter by CD8+ cell
load("ZZM_NSCLC_TPMdata.RData")
cell <- read.table("ZZM_NSCLC_celldata.txt",sep = "\t",header = F)

cell <- cell[which(str_sub(cell$V3,1,3) == "CD8"),]
cell$V1 <- str_replace(cell$V1,"-",".")
raw <- raw[which(rownames(raw) %in% cell$V1),]
...
write.csv(plotdata,file = "NSCLC_CD8+_plotdata_fin_20220518.csv")
write.csv(proliferation,file = "ProliferationNSCLC_gene.csv")
